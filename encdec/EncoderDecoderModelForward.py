...
parameter_dict = {}
train_path = "../Data/"
parameter_dict["source"] = train_path + "player_input" # 読み込む文章を記述するファイル
parameter_dict["target"] = train_path + "player_output" # 出力を保存するファイル→TODO: 文脈をとる
parameter_dict["test_source"] = train_path + "player_1_test"
...
for i in range(100):
    # parameter_dict["source"] を読み込み、予測文を出力する
    # FIXME: #testとの相違点は初期読み込みの出力をなくし、末尾の返答のみ出力するようにした。複数文を加味することを目指す
    if i > 0: encoderDecoderModel.reply()
    # parameter_dict["source"] を更新する
    gens.input_file(parameter_dict["source"])
